...
resource kubernetes_persistent_volume qbittorrent {
...
resource kubernetes_persistent_volume_claim qbittorrent {
  count = length(var.qbittorrent_nfs)
  metadata {
    name      = values(var.qbittorrent_nfs)[count.index].name
  ...
  spec {
    ...
    volume_name = kubernetes_persistent_volume.qbittorrent[count.index].metadata.0.name
  }
}
